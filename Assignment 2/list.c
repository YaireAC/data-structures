...
/*
*Meanto help us with the stack_top function in stack. To avoid
*shouldn't do stuff like stack->list->head in stack.c since the members of the list (head, the node struct) 
are hidden from the stack to maintain encapsulation 
*
*Params:
*   list - the linked list to be reversed.  May not be NULL.  When this
*     function returns this should contain the reversed list.
...
void* list_front(struct list* list) {
    if (list == NULL || list->head == NULL) {
        return NULL; // Check if list is NULL or empty
    }
    return list->head->val; // Return the value of the head node
}
