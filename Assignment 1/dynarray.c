...
struct dynarray* dynarray_create()
{
    struct dynarray* new_array = malloc(sizeof(struct dynarray)); //meant to be the new array
                                    //calculates the size in bytes needed to store an object type "struct dynarray"
    ...
    new_array->data = malloc(sizeof(void*) * new_array->capacity);
                        //calculates the size of a pointer on the system. The void* type is a generic pointer type
                        //and sizeof(void*) returns the size in bytes of a pointer on the current system.
    return new_array;
